function printCheckinSheet(levelName = 'Beginner') {
  ...
  peopleArr.forEach(person => {
    buffer.push([person.sfn, person.sln, person.school.toString(), person.vRing.toString()])
  })
 
  targetSheet.getRange(1,1,buffer.length, 4).setValues(buffer).setNumberFormat("@")
...
function getVirtRing(peopleArr, vRing, checkForms = false, checkSparring = false) {

  var retPeopleArr = []

  retPeopleArr = peopleArr.filter(person => {person[vRing] == vRing})

  if (checkForms) {
    retPeopleArr = retPeopleArr.filter(person => {person[form]})
  }

  if (checkSparring) {
    retPeopleArr = retPeopleArr.filter(person => {person[sparring]})
  }
  return retPeopleArr
}
